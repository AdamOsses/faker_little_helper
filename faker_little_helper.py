...
while True:
    ...
    if bracket_r_pos > bracket_l_pos:
        ...
        columns_data = selected_column_raw[:bracket_l_pos+1] + selected_column_raw[bracket_r_pos:]
        ...
        print(f'brackert_data->{bracket_data}<-')
        print(f'elem->{elem}<-')
        for e in elem:
            if e.isdigit():
                row_count += int(e)
                ...
                try:
                    ...
                    locale.append(e)
    print(f'*** File(s): {files if len(files)>0 else "file.csv"}')
...
fake = Faker(locale)
file = open(files[0], 'w', newline='')    # add list of files
